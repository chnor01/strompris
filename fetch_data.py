...
class StromprisClient:
    ...
    def _build_url(self, day: date):
        return (
            f"{self.BASE_URL}/{day.year}/{day.month:02d}-{day.day:02d}_{self.area}.json"
        )
    ...
    def _convert_data(self, data: list[dict]):
        ...
        df = df.rename(
            columns={"NOK_per_kWh": "pris_nok_kwh", "EUR_per_kWh": "pris_eur_kwh"}
        )
...
class FrostClient:
    ...
    def __init__(
        ...
        self.client = httpx.Client(
            transport=transport, auth=(client_id, ""), timeout=timeout
        )
    ...
    def _parse_offset(offset: str):
        ...
        return (
            pd.Timedelta(hours=int(match.group(1))) if match else pd.Timedelta(hours=0)
        )

    def _data_path(self, start: date, end: date):
        return (
            self.data_dir
            / f"{self.station_id}_{start.isoformat()}_{end.isoformat()}.parquet"
        )
...
class NveClient:
    ...
    def _convert_data(self, data: list[dict]):
        ...
        return (
            df[["valid_time", "fyllingsgrad", "fylling_TWh", "kapasitet_TWh"]]
            .sort_values("valid_time")
            .reset_index(drop=True)
        )
...
def merge_data(
    strompriser: pd.DataFrame, weather: pd.DataFrame, magasin: pd.DataFrame
) -> pd.DataFrame:
...
def fetch_data_range(
    ...
    if not client_id:
        raise ValueError(
            "FROST_CLIENT_ID er ikke satt — sjekk at .env lastes inn før dette kjøres"
        )
    ...
    print(
        f"Saved {len(merged)} rows for time range: ( {start} to {end} ) -> {output_path}"
    )
