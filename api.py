...
class ChargingRequest(BaseModel):
    ...
    hours_ahead: int = Field(
        default=24, ge=1, le=72, description="How many hours ahead the deadline is"
    )
    max_kwh_per_hour: float = Field(
        default=11.0, gt=0, description="Charger's power limit in kWh/hour"
    )
...
def get_forecast(hours_ahead: int = 24):
    ...
    if hours_ahead < 1 or hours_ahead > 72:
        raise HTTPException(
            status_code=400, detail="hours_ahead must be between 1 and 72"
        )
