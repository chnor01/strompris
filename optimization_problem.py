...
class ChargingPlan:
    ...
    def summary(self) -> str:
        lines = [f"Charging plan - total {self.total_kwh:.1f} kWh, cost {self.total_cost:.2f} kr\n"]
        for t, kwh, price in zip(self.hours, self.kwh_per_hour, self.prices):
            if kwh > 0.01:
                lines.append(f"  {t.strftime('%Y-%m-%d %H:%M')}: {kwh:.2f} kWh @ {price:.3f} kr/kWh")
        return "\n".join(lines)
...
def optimize_charging(
    ...
    charging = [pulp.LpVariable(f"charging_{i}", lowBound=0, upBound=max_kwh_per_hour) for i in range(n)]
...
def naive_charging(price_forecast: pd.DataFrame, kwh_needed: float, price_col="pris_nok_kwh"):
...
def make_price_forecast(
    ...
    for i in range(1, n_hours_ahead + 1):
        ...
        working_data = pd.concat([working_data, new_row.to_frame().T], ignore_index=True)
...
def _build_future_row(working_data: pd.DataFrame, new_time: pd.Timestamp) -> pd.Series:
    ...
    row["temp_rullende_24h"] = np.mean([get_temp(h) for h in range(0, 24)])
    row["temp_rullende_72h"] = np.mean([get_temp(h) for h in range(0, 72)])
    row["temp_min_72h"] = np.min([get_temp(h) for h in range(0, 72)])
    row["temp_endring_24h"] = last_known_weather["temperatur_c"] - get_temp(24)
    row["vind_rullende_24h"] = np.mean(
        [wind_series.get(new_time - pd.Timedelta(hours=h), last_known_weather["vind_m_s"]) for h in range(0, 24)]
        )
 
    fill_level_series = working_data.set_index("timestamp")["fyllingsgrad"]
    fill_level_week_ago = fill_level_series.get(new_time - pd.Timedelta(hours=168), last_known_weather["fyllingsgrad"])
    row["fyllingsgrad_endring_7d"] = last_known_weather["fyllingsgrad"] - fill_level_week_ago
