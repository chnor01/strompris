...
st.caption(
    "NO1 electricity prices predicted with LightGBM, load scheduling optimized with linear programming"
)
...
def get_forecast(hours_ahead: int):
    res = httpx.get(
        f"{API_URL}/forecast", params={"hours_ahead": hours_ahead}, timeout=30
    )
    res.raise_for_status()
    return res.json()
...
kwh_needed = st.sidebar.number_input(
    "Energy needed (kWh)", min_value=1.0, max_value=200.0, value=40.0, step=1.0
)
hours_ahead = st.sidebar.slider(
    "Deadline (hours ahead)", min_value=1, max_value=72, value=24
)
max_kwh_per_hour = st.sidebar.number_input(
    "Max charging power (kWh/hour)", min_value=1.0, max_value=50.0, value=11.0, step=0.5
)
...
with col2:
    ...
    st.metric(
        "Lowest price in period", f"{forecast_df['pris_nok_kwh'].min():.3f} kr/kWh"
    )
    st.metric(
        "Highest price in period", f"{forecast_df['pris_nok_kwh'].max():.3f} kr/kWh"
    )
...
if compute:
    ...
    m3.metric(
        "Savings", f"{plan['savings_kr']:.2f} kr", delta=f"{plan['savings_pct']:.1f}%"
    )
    ...
    st.info(
        "Set energy needed and deadline in the sidebar, then click 'Compute charging plan'."
    )
